...
export default function SectionsPage() {
  ...
  const handleAddFiles = async (e) => {
    ...
    const imagesToAdd = buffers.map((src) => ({ id: generateId("img"), src }));

    setSections((prev) =>
      prev.map((s) =>
        s.id === activeSectionId ? { ...s, images: [...s.images, ...imagesToAdd] } : s
      )
    );
  ...
  return (
    <div className="space-y-6 pb-10 p-4 md:p-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-2 md:gap-4 mt-2">
        ...
        <label className="cursor-pointer">
          ...
          />
          <span className="px-3 py-2 bg-orange-500 text-white rounded text-sm h-10 flex items-center justify-center w-full md:w-auto">
            Add Images
          </span>
        </label>
      </div>
